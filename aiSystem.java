...
public class aiSystem {
    ...
    //ROBOT STATICS
    int robot_y = 1050; //1050 //720
    int robot_x = 980; //980 //720
    ...
    //WHEEL VELOCITIES
    public void find_wheel_velocities(int ballX,int ballY,Mat rgbaImage,double [] obstacles){
        ...
        for(int j = 0; j < residual_field.length; j++){
            residual_field[j] = Math.max(0, attractive_field[j] - repulsive_field[j]);
        }
        ...
        double vel_rotate = (Math.min(MaxRobotRotate, Math.max((-MaxRobotRotate),(goalError*GoalP))));
        double vel;
        if(ballX > 10 && ballY >10) {
        ...
        if(ballX == robot_x && ballY == robot_y){
            ball_reached = 1;
        }
        ...
        if(debug) {
            ...
            Imgproc.putText(rgbaImage, steve, new Point(40, 40), Core.FONT_HERSHEY_SIMPLEX, 1, green, 2);
            Imgproc.putText(rgbaImage, peteo, new Point(40, 80), Core.FONT_HERSHEY_SIMPLEX, 1, green, 2);
            Imgproc.putText(rgbaImage, steveo, new Point(40, 120), Core.FONT_HERSHEY_SIMPLEX, 1, green, 2);
            Imgproc.putText(rgbaImage, toe, new Point(40, 160), Core.FONT_HERSHEY_SIMPLEX, 1, green, 2);
            Imgproc.putText(rgbaImage, rod, new Point(40, 200), Core.FONT_HERSHEY_SIMPLEX, 1, green, 2);
            if(ballX > 0 && ballY > 0) {
