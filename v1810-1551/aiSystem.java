...
public class aiSystem {
    ...
    //FIND BALL
    public boolean find_ball(int ballX,int ballY, Mat rgbaImage){
        boolean found = false;
        if(ballX == 0 && ballY == 0){
            goal_angle = 200;
    ...
    public void state_machine(int ballX,int ballY,int goalX,int goalY, Mat rgbaImage,double [] obstacles){
        ...
        if(ball_reached == 1) { //has ball been reached?
            if (kicked == 0) { //cancel loop
                if (goalX < 900 && goalX > 0) { //is goal in site and more than 0
                    if(count > 5) { //count 5 frames to make sure u are looking at goal.
                        kick = 1; //kick ball
                    }
                    count++; //counter
                }
                ...
                if(kick == 1){ //if we are kicking
                    Imgproc.putText(rgbaImage, "i am kicking", new Point(40, 190), Core.FONT_HERSHEY_SIMPLEX, 0.8, green, 2);
                    if (kickerator > 20) { //wait 20 frames whilst sending ones
                        kick = 0; //stop kicking
                        kicked = 1; //cancel out of this loop
                    }
                    kickerator++; //counter
                }
            }
