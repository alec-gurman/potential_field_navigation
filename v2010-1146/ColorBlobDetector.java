...
public class ColorBlobDetector {
    ...
    private Scalar obstacle_upper = new Scalar(60,255,50,0); //hue was 50
    private Scalar goal_lower = new Scalar(15,230,125,0);
    private Scalar goal_upper = new Scalar(25,255,255,0);
    private Scalar second_goal_lower = new Scalar(90,0,0,0);
    private Scalar second_goal_upper = new Scalar(179,120,200,0);
    ...
    private Mat goal_hierarchy_blue = new Mat();
    ...
    private Mat goal_image_second = new Mat();
    ...
    private double goal_area = 0;
    private int blue_goal_x = 0;
    private int blue_goal_y = 0;
    ...
    double goal_maxArea = 200;
    double ball_maxArea = 25;
    double obstacle_maxArea = 1000; //was 1500

    //ARRAYS
    double [] obstacle_array = new double [1000];

    public void Configure(Mat Image,int ball_is_reached){
        ...
        find_goal(Image, hsv_image,ball_is_reached);
        find_goal_blue(Image,hsv_image,ball_is_reached);
        find_ball(Image, hsv_image,ball_is_reached);
        find_obstacles(Image,hsv_image);
        //find_wall(Image,hsv_image);
    }

    public void find_ball(Mat rgbaImage,Mat Timage,int ballStatus) {
        int store = 0;
    ...
    public void find_goal(Mat rgbaImage,Mat Timage,int ballStatus) {
        int store = 0;
        ...
        for(int i = 0; i < contours.size();i++){
            if (Imgproc.contourArea(contours.get(i)) > goal_maxArea) {
                goal_area = Imgproc.contourArea(contours.get(i));
    ...
    public void find_goal_blue(Mat rgbaImage,Mat Timage,int ballStatus) {
        int store = 0;
        Core.inRange(Timage, second_goal_lower, second_goal_upper, goal_image_second); //Threshold the image in the colour required ( in this case its Orange )
        Imgproc.erode(goal_image_second, goal_image_second, Imgproc.getStructuringElement(Imgproc.MORPH_RECT, new Size(2,2))); // Erode Thresholded Image
        Imgproc.dilate(goal_image_second, goal_image_second, Imgproc.getStructuringElement(Imgproc.MORPH_RECT, new Size(2,2))); // DIlate Thresholded Image
        List<MatOfPoint> contours = new ArrayList<MatOfPoint>(); // Create array to put contours into
        Imgproc.findContours(goal_image_second, contours, goal_hierarchy_blue, Imgproc.RETR_EXTERNAL, Imgproc.CHAIN_APPROX_SIMPLE); // Find Existing Contours
        ...
        for(int i = 0; i < contours.size();i++){
            if (Imgproc.contourArea(contours.get(i)) > goal_maxArea) {
                ...
                Moments p = Imgproc.moments(draw_contours.get(store), false);
                blue_goal_x = (int) (p.get_m10() / p.get_m00());
                blue_goal_y = (int) (p.get_m01() / p.get_m00());
                store++;
            }
        }
    }
    ...
    public void find_obstacles(Mat rgbaImage, Mat Timage) {
        ...
        Rect roi = new Rect(0, 0, 300, 179); //galaxy s3 //cropped to 980px
    ...
    public int ball_coordsX() { return ball_x; }

    public int ball_coordsY() { return ball_y; }

    public int goal_coordsX() { return goal_x; }

    public int goal_coordsY() { return goal_y; }

    public int blue_goal_coordsX() { return blue_goal_x; }

    public int blue_goal_coordsY() { return blue_goal_y; }

    public double goal_area() { return goal_area;}
