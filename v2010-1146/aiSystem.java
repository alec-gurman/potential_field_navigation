...
public class aiSystem {
    ...
    boolean state_3_was_active = false;
    ...
    public void state_machine(int ballX,int ballY,int goalX,int goalY,double goalArea,Mat rgbaImage,double [] obstacles){
        ...
        if(ballX == 0 && ballY == 0 && !state_8_kicking){
            ball_reached = 0;
            state_3_active = false;
            if(state_3_was_active && !state_3_active){
                state_3_was_active = false;
                state_3_avoiding = true;
            }
            if(previous_goal_angle > 0) {
                if (previous_goal_angle < 360 && previous_goal_angle > 270) {
                    ...
                    if(state_3_avoiding){
                        ...
                        store_wheel_velocities(120.0,140.0);
                    }
                    else{
                        find_wheel_velocities(headingAngle, rgbaImage,0.7,0.7);
                    }
                }
...
//        }
        ...
        /********** STATE 3.5 - AVOIDING OBSTACLE **********************/

        if(state_3_active){
            state_3_was_active = true;
        }

        if(state_3_avoiding){
            if(state_3_count > 5){
                state_3_avoiding = false;
                state_3_count = 0;
            }
            state_3_count++;
        }
        ...
        if(ballX < 150 && ballX > 0 && obstacles[1] == 0 && obstacles[2] == 0 && goalX == 0 && goalY == 0 && !state_8_kicking){
            ...
            //goal_angle = containAngle360(Math.toDegrees(goal_rad)); //limit angle between 0 and 360 degrees
            store_wheel_velocities(100.0,30.0); //was 80,20 (working aiight)
            state = 4; //for debug
            dribble = 1;
            kick = 0;
        }
